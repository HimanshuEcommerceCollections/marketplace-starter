...
export function ServiceForm({ mode, initial }: ServiceFormProps) {
  ...
  function applyApiError(err: ServiceApiError) {
  ...
  async function submit(publish: boolean) {
    ...
    try {
      ...
      if (mode === "edit") {
        const result = await updateService(initial!.id, {
          name: name.trim(),
          slug: slug || undefined,
      ...
      if (err instanceof ServiceApiError) applyApiError(err);
  ...
  return (
    <Card className="p-5 sm:p-6">
      ...
      >
        ...
        <Field label="Name" htmlFor="svc-name" error={fieldError("name")} required>
          <Input
            id="svc-name"
            value={name}
        ...
        <Field
          ...
          hint="Starting price for the service."
          required
        >
          <Input
            id="svc-price"

...
import {
  ...
  deleteServiceAssets,
  deleteServiceCover,
  reorderServiceCovers,
  coverFilename,
  type ServiceAssets,
  type UploadProgress,
} from "@/lib/admin/service-assets";
import {
  ICON,
  COVER,
  kb,
...
export function ServiceAssetsPanel({ slug }: { slug: string }) {
  const [assets, setAssets] = React.useState<ServiceAssets | null>(null);
  ...
  const apply = React.useCallback((next: ServiceAssets) => {
    setAssets(next);
  ...
  const load = React.useCallback(async () => {
    ...
    setLoadError(null);
    try {
      apply(await getServiceAssets(slug));
    } catch (err) {
      setLoadError(
        err instanceof ServiceApiError ? err.message : "Failed to load assets.",
      );
    } finally {
      setLoading(false);
    }
  ...
  // POST (create) when the service has no assets yet; PUT (update) thereafter.
  function sendAssets(form: FormData, opts?: UploadProgress) {
    const empty = !assets?.iconPath && (assets?.coverImages.length ?? 0) === 0;
    return (empty ? createServiceAssets : updateServiceAssets)(slug, form, opts);
  }
  ...
  async function removeIcon() {
    ...
    await run(() => updateServiceAssets(slug, form), "Icon removed.");
  }
  ...
  async function deleteCover(coverUrl: string) {
    await run(
      () => deleteServiceCover(slug, coverFilename(coverUrl)),
      "Cover image deleted.",
    );
  }
  ...
  async function persistOrder(next: string[]) {
    ...
    const ok = await run(() => reorderServiceCovers(slug, next), "Cover order updated.");
  ...
  async function deleteAll() {
    if (
      !window.confirm(
        "Delete the icon and all cover images for this service? This cannot be undone.",
      )
    ) {
      return;
    }
    const ok = await run(() => deleteServiceAssets(slug), "All assets deleted.");
    if (ok) {
      clearPendingIcon();
      setPendingCovers([]);
    }
  ...
  return (
    <span className="flex size-16 items-center justify-center rounded-lg border border-border bg-muted text-muted-foreground">
      {url ? (
        ...
        <img src={url} alt="Service icon preview" className="size-9" />
      ) : (

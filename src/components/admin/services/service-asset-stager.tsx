...
/**
 * Controlled staging UI for a service's icon + cover images, used in the
 * create-service flow. It only SELECTS and previews files (client-side
 * validated); the parent uploads them after the service is created, because
 * the asset API is keyed by slug and requires the service to exist first.
 */
export interface ServiceAssetStagerProps {
  icon: File | null;
...
export function ServiceAssetStager({
  icon,
  onIconChange,
  covers,
  onCoversChange,
    ...
    <div className="flex flex-col gap-5 rounded-lg border border-border bg-muted/30 p-4">
      <div>
        ...
        <p className="mt-0.5 text-xs text-muted-foreground">
          Added after the service is created. You can also manage them later from
          the service page.
        </p>
      </div>
